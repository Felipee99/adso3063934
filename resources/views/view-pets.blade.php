...
<body class="min-h-screen bg-gradient-to-br from-teal-900 via-indigo-900 to-purple-900 flex flex-col items-center justify-center">
  ...
  <div class="card-body">
    ...
    @if($pet->kind == 'dog')
    <div class="badge badge-error">Dog</div>
    @elseif($pet->kind == 'cat')
    <div class="badge badge-info">Cat</div>
    @elseif($pet->kind == 'bird')
    <div class="badge badge-success">bird</div>
    @elseif($pet->kind == 'rabbit')
    <div class="badge badge-warning">rabbit</div>
    @endif
    <p class="w-[200px]">{{ $pet->description }}</p>
    <div class="card-actions justify-end">
      <a class="text-white bg-teal-700 rounded-full p-1 hover:scale-150 transition-all" href="{{ url('view/pet/'.$pet->id) }}">
