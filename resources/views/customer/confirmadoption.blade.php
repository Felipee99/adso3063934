...
<div class="bg-[#0009] p-10 rounded-sm">
    ...
    <div class="avatar flex flex-col cursor-pointer gap-1 hover:scale-110 transition ease-in justify-center items-center">
        <div class="mask mask-squircle w-60">
            <img src="{{ asset('image/' . ($pet->image ?? 'no-image.png')) }}" alt="Pet image" />
        </div>
    </div>
    ...
    <form id="frmAdopt" method="POST" action="{{ url('makeadoptions/' . $pet->id) }}" class="mt-6">
        @csrf
        <button id="btnAdopt" type="submit" class="btn btn-primary w-full">
            <span id="btnLabel">Make adoption</span>
        </button>
    </form>
</div>
<!-- Modal -->
<dialog id="adoptModal" class="modal">
  <div class="modal-box bg-[#0009] backdrop-blur-md text-white">
    <h3 class="text-2xl font-bold">🐶 Adoption completed!</h3>
    <p class="py-4">Your adoption was successfully made. Melo, melo. 😎</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('frmAdopt');
        const btn = document.getElementById('btnAdopt');
