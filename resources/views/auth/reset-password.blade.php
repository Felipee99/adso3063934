...
@section('content')
    ...
    Password Reset
            ...
            <form method="POST" action="{{ route('password.store') }}" class="card-body">
                ...
                <input type="text" class="input bg-[#0009] w-full" name="email" placeholder="Email@example.com" value="{{ old('email') }}" />
                @error('email')
                    <small class="badge badge-error w-full mt-1 py-6">{{ $message }}</small>
                @enderror

                  <label class="label">Password:</label>
                <input type="password" class="input bg-[#0009] w-full" name="password" placeholder="New Password" value="{{ old('password') }}" />
                @error('password')
                    <small class="badge badge-error w-full mt-1 py-6">{{ $message }}</small>
                @enderror

                <label class="label">Password Confirmation:</label>
                <input type="password" class="input bg-[#0009] w-full" name="password_confirmation" placeholder="Confirm New Password" value="{{ old('password_confirmation') }}" />
